...
import React, { useActionState, useEffect, useMemo, useTransition, useRef } from "react";
...
const normalizePhoneNumber = (value: string): string => {
  ...
  } else if (digits.length > 0) { // Doesn't start with +
    if (digits.startsWith('1') && digits.length > 1) { // Starts with 1 (likely US country code)
      digits = '+' + digits;
    } else { // Assumed to be a local number, prepend +1
      digits = '+1' + digits;
    }
  }
...
export default function BurritoOrderForm() {
  ...
  const watchedSelectedBurritos = useWatch({ control: form.control, name: "selectedBurritos" }) || [];
    ...
    if (!areArraysEqual(currentSelected, prevSelected)) {
      setQuantities(prevQuantities => {
        ...
        currentSelected.forEach(burritoName => {
          newQuantities[burritoName] = prevQuantities[burritoName] || 1;
        });
    ...
    if (formState?.success) {
      ...
      if (formState.errors) {
        if (formState.errors.name)
          form.setError("name", { type: "server", message: formState.errors.name[0] });
        if (formState.errors.email)
          form.setError("email", { type: "server", message: formState.errors.email[0] });
        if (formState.errors.phoneNumber)
          form.setError("phoneNumber", { type: "server", message: formState.errors.phoneNumber[0] });
        if (formState.errors.burritoOrders)
          form.setError("selectedBurritos", { type: "server", message: formState.errors.burritoOrders[0] });
      }
    }
      ...
      <CardHeader>
        ...
        <div className="text-center border-t border-border pt-4">
          ...
          <h2 className="text-xl font-semibold text-primary tracking-tight mt-1">
            Mojo Lime Shredded Pork Burrito w/ Smoky Corn Salsa & Avocado Crema
          </h2>
        </div>
      </CardHeader>
      <Form {...form}>
        ...
        >
          <CardContent className="space-y-6">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>
                    I&apos;ll use this to tell you about future burritos, no spam I
                    promise.
                  </FormDescription>
                  <FormMessage />
                </FormItem>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>
                    Used to tell you when your order is ready for pickup. Must be a US number in the format +1XXXXXXXXXX.
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field: { onChange, value, ...restField } }) => ( // Destructure `value` here
                <FormItem>
                  <FormLabel>
                    Which Burrito(s)?{" "}
                  ...
                  <FormControl>
                    <div className="space-y-3">
                      {burritoOptions.map((burritoOption) => {
                        const fieldId = `checkbox-${burritoOption.replace(/\W/g, '-')}`;
                        const isChecked = value?.includes(burritoOption); // Use `value` from render prop

                        return (
                          <div key={burritoOption} className="flex flex-row items-start space-x-3 space-y-0">
                             <Checkbox
                              id={fieldId}
                              checked={isChecked}
                              onCheckedChange={(checkedState) => {
                                const currentSelection = value || []; // Use `value`
                                let newSelection;
                                if (checkedState) {
                                  newSelection = [...currentSelection, burritoOption];
                                } else {
                                  newSelection = currentSelection.filter(
                                    (name) => name !== burritoOption
                                  );
                                }
                                onChange(newSelection); // Call onChange from render prop
                                form.trigger("selectedBurritos"); // Manually trigger validation
                              }}
                              aria-labelledby={`label-${burritoOption.replace(/\W/g, '-')}`}
                              {...restField} // Pass down other field props
                            />
                            <Label
                              htmlFor={fieldId}
                              id={`label-${burritoOption.replace(/\W/g, '-')}`}
                  ...
                  <FormMessage>
                    {form.formState.errors.selectedBurritos?.message}
                  </FormMessage>
                </FormItem>
              )}
            />
            ...
            <div className="text-sm text-muted-foreground mt-2">
              *Burrito of the Week Price will vary weekly based on ingredient
              cost.
            </div>

            {anySelected && (
              <div className="space-y-4 pt-4 border-t border-border transition-all duration-300 ease-in-out">
                <h3 className="text-md font-semibold text-primary">Quantity</h3>
                {watchedSelectedBurritos.map((burritoNameWithPrice) => {
                  const quantity = quantities[burritoNameWithPrice] || 1;
                  return (
                    <div
                    ...
                    >
                      <Label
                        htmlFor={`quantity-input-${burritoNameWithPrice.replace(/\W/g, '-')}`}
                        className="flex-1"
                      >
                        {burritoNameWithPrice}
                      </Label>
                      <Input
                        id={`quantity-input-${burritoNameWithPrice.replace(/\W/g, '-')}`}
                        type="number"
                        min="1"
                        value={quantity}
                        onChange={(e) =>
                          handleQuantityChange(burritoNameWithPrice, e.target.value)
                        }
                        className="w-20"
                        aria-label={`Quantity for ${burritoNameWithPrice.split(" - ")[0]}`}
                        required
                      />
                    </div>
                  );
                })}
                {formState?.errors?.burritoOrders && !form.formState.errors.selectedBurritos && (
          ...
          <CardFooter>
            <SubmitButton isPending={form.formState.isSubmitting || isTransitionPending} />
          </CardFooter>
        </form>
      </Form>
    </Card>
